 ...
 * observes the same equity. risk-svc threads the equity into the {@code notional_cap_pct_of_equity}
 * gate. Any exception is treated as fail-closed by the caller (rejecting the entry), mirroring the
 * kill-switch and pre-trade-check reads.
 *
 ...
 * notional-cap gate is also opt-in via {@code StrategyConfig.notional_cap_pct_of_equity}, so the
 * cross-service round-trip only fires when the strategy explicitly enabled it.
 */
@ActivityInterface
