...
- `services/exec/.../activities/PortfolioHistoryExecActivityImpl.java:46-58` — after the history read, if `timestamps` non-empty, call `broker.getAccountActivities(timestamps[0], timestamps[last])`, set the two parallel arrays + `cashFlowsAvailable=true`. Wrap in `try/catch (RuntimeException | UnsupportedOperationException)` → on failure `log.warn` (so a production "—" is diagnosable) and set `cashFlowsAvailable=false`, empty flow arrays (degrade; StubBroker / non-Alpaca targets simply report unavailable). **No new Temporal command** — this is a second broker call *inside the same existing Activity*, so `PortfolioHistoryWorkflow`'s command sequence is unchanged → **no `getVersion` gate needed** and existing (sub-8s, request-scoped) histories replay byte-identically.

**Day-granularity alignment (review follow-up — this is the whole bug, narrowed):** the two timestamp series are NOT the same granularity. Non-trade activities (CSD/CSW/JNLC) carry only a calendar `date`, which `epochSecondsOf` parses to **midnight UTC**; portfolio-history bars are at **market time** (~20:00Z). So for a range whose FIRST day contains a transfer, `epoch(date) < t0` — the exec query (`after=<t0 instant>`) omits the row outright, and the BFF's exact `t < t0` filter would drop it even if returned. The deposit then lands right back in `EV − BV` as profit: the +945% class of bug, narrowed to ranges that start on a transfer day. Fixed at BOTH ends: exec queries whole UTC days (`after` floored to t0's midnight, `until` pushed to the day after t1 — over-fetching is safe because the BFF filter is authoritative), and the calculator floors its lower bound to t0's UTC day while leaving the upper bound at exactly t1 (a same-day flow is at midnight, hence always ≤ t1; widening it would pull in days the equity series doesn't cover). The Modified-Dietz weight is clamped to 1 so a first-day flow (raw ratio > 1) isn't over-weighted in the denominator.

**Pagination (review follow-up):** Alpaca pages `/v2/account/activities`. A single unpaged GET truncates at the page size, and a dropped deposit is indistinguishable from no deposit — it reads as profit. The adapter now walks pages (`page_size=100`, `page_token` = previous page's last `id`) and, if the walk can't complete within 100 pages or the cursor stops advancing, **throws** rather than returning a partial list: the caller degrades to `cash_flows_available=false` and the UI shows "—". Silently understating `netFlows` is the one outcome that must not happen.

**Shared-timeout mitigation (review follow-up):** the two broker calls share ONE 15s `StartToCloseTimeout` (`PortfolioHistoryWorkflowImpl:33`), and `AlpacaConfig.buildRestClient` sets no connect/read timeout. An unbounded *slow* (not erroring) `/v2/account/activities` would therefore burn the whole budget and make Temporal time out and retry the **entire** Activity — including the already-successful `portfolio_history` read this lookup was only meant to augment, and without ever reaching the degrade `catch`. Mitigation: `getAccountActivities` runs on a dedicated `activitiesClient` (`AlpacaPaperBroker`) derived via `RestClient#mutate()` with a `JdkClientHttpRequestFactory` bounded at **2s connect / 5s read**; a slow endpoint fails fast, the caller degrades, and the chart survives. The shared `client` is deliberately left unbounded — a read timeout on the ORDER path would turn an order Alpaca actually accepted into a retry → duplicate. Read-path failures also get their own `AccountActivitiesReadError` type instead of the order-worded `mapError`, so a failed READ is never logged as "Alpaca rejected order".

**Tests (TDD):**
- `services/exec/.../broker/alpaca/AlpacaPaperBrokerTest` (WireMock, mirror the portfolio-history test): stub `/v2/account/activities` with a CSD + CSW + JNLC → asserts the URL carries `activity_types=CSD,CSW,JNLC` and `after`/`until`, and the mapped `AccountCashFlow` list has correct signs/epochs; an HTTP 500 whose body carries an order-path token → `AccountActivitiesReadError` with read wording, NOT "rejected order"; a delayed response against a short-timeout broker → fails fast (bounded), proving the shared-budget mitigation.
- `services/exec/.../activities/PortfolioHistoryExecActivityImplTest`: broker returns history + flows → result carries both parallel arrays and `cashFlowsAvailable=true`; `getAccountActivities` throws → `cashFlowsAvailable=false`, empty flows, history still returned (degrade); empty history window → lookup skipped, `cashFlowsAvailable` never set.
