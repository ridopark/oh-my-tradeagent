...
- `CopytradeSignalWorkflowImpl.java:991-996` — keep `matchResult` / keyword `fraction` exactly as-is (the fallback). Add `effectiveFraction`:
  ...
  - mapping when `enforce && intent != null`:
    - `intent == "full"` → `1.0` (fixes the under-close incident)
    - `intent == "partial"` → `keywordFraction < 1.0 ? keywordFraction : defaultStcFraction(config)` (demotes a keyword-1.0 over-close like `"partial. Taking profit…"`; otherwise keeps the keyword-sized partial)
  - else → `keywordFraction` (unchanged).
...
- `stc_enforceOn_partialIntent_demotesKeywordFullClose` — `"partial. Taking profit as it comes"`, `close_intent="partial"`, keyword would give 1.0 → `fraction == defaultStcFraction` (over-close fixed).
...
- **Rollout gate (shadow → enforce):**
  ...
  3. **GO/NO-GO:** only if forward `recall[full]` rises and no partial-recall regression → flip `stc_intent_enforce=true` **per live tenant via DB CAS** (live-tenant config is an operator edit, NOT a repo YAML — `staging_paper`/`prod_real`/`prod-kipark` strategy YAMLs are cluster-only). Order: **staging_paper first**, bake, then prod_real, then prod-kipark.
