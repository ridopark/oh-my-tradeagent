...
| **7. Live broker promotion** | Wire the chosen provider's live adapter (`alpaca-live`, `tradier-live`, `ibkr-live`, or `schwab-live`) on its own task queue; manual promotion flow (operator sign-off — see Issue #23, see Issue #18); per-strategy capital allocator producing live `contracts_per_signal`; live-only alerting (PagerDuty / Slack). **Promotion gate explicitly excludes sandbox/paper P&L (Sharpe, win-rate, return) as a criterion** — broker sandboxes (Tradier, Alpaca paper, etc.) deliver instant marketable fills, no realistic slippage, stale/synthesized option quotes, no intraday IV updates, no halt simulation, no partial fills, and no PFOF routing artifacts, so paper P&L is not predictive of live P&L (Issue #18, quant-analyst review). The gate uses **operational metrics only**: (a) zero double-fills, (b) zero orphan reconciliations, (c) audit completeness, (d) signal-to-order latency p50/p99 within target. Promotion is gated behind a mandatory **"shadow live" sub-phase**: 1 contract on the real broker (`*-live` adapter, smallest tradable size) for **N=10 consecutive trading days** with the four operational metrics holding green every day; only after the shadow-live window closes clean may sizing scale up. Rollback path remains revert `broker_target` in YAML to the paper variant. | Shadow-live sub-phase completed: 1 contract on the real broker for 10 consecutive trading days with zero double-fills, zero orphan reconciliations, complete audit log, and signal-to-order latency p50/p99 within target across every day of the window. Sandbox/paper P&L is **not** consulted. After the shadow-live window closes clean, first tenant + strategy is promoted under small-size constraints and a live trade executes successfully. Rollback path tested (revert `broker_target` in YAML to the paper variant; recon catches any in-flight live order). |
