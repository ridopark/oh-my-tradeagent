...
| **Multi-tenant isolation** | Workflow ID prefix enforced; Activity reading credentials without `SecretsResolver(tenant_id)` fails CI guardrail; cross-tenant query returns empty (not other-tenant's data). |
