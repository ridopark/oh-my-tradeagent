...
3. **Run the Java services** (each via `mvn`, sourcing the same env so they use the local Alpaca
   account + output webhook). DB hosts + Temporal default to `localhost`; the boot-required bits
   (`EXEC_DB_URL`, `ORCHESTRATOR_DB_USER/PASS` superuser override, `DASHBOARD_READONLY_PASSWORD`,
   `APCA_*`) all come from `.env.local`:
   ```sh
