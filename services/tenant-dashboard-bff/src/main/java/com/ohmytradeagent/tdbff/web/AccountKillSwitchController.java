 ...
 *
 * <ul>
 *   <li>{@code GET /api/account-killswitch} — current state + the server-authoritative {@code
 *       resettable} flag.
...
public class AccountKillSwitchController {
   ...
   * Computed off the workflow's {@code tripped_at}; enforced on both the state read ({@code
   * resettable}) and the reset write (409 {@code circuit_breaker_active}).
   */
  ...
  public ResponseEntity<Map<String, Object>> reset(
    ...
    // Single-operator, threshold-immutable reset: the DTO carries only
    // schema_version/approver_id_1/
    // note, so a tenant cannot smuggle a loss-cap threshold through the reset. The honored reset
    // audits (via the workflow) as KillSwitchResetApproved with approver_id_1="tenant:"+tenant.
