 ...
 * the paper DB), this one routes paper vs live so the list can show live accounts too.
 *
...
public class AdminTenantAccountReader {
  ...
  private final DSLContext paperDsl;
  private final DSLContext liveDsl;

  public AdminTenantAccountReader(
      @Qualifier("execAlpacaPaperDsl") DSLContext paperDsl,
      @Qualifier("execAlpacaLiveDsl") DSLContext liveDsl) {
    this.paperDsl = paperDsl;
    this.liveDsl = liveDsl;
  }

  /**
   * The raw {@code expected_account_id} for (tenant, broker_target), or {@code null} when no
   * credential row exists or the column is null. Fail-soft: never throws on a missing row. The
   * CALLER masks; this returns the raw account so the mask lives in one place (the controller's
   * mapper). It is NOT a secret — it is the broker account number, surfaced masked.
   */
  public String accountId(String tenantId, String brokerTarget) {
    if (brokerTarget == null) {
      return null;
    }
    DSLContext dsl = isLive(brokerTarget) ? liveDsl : paperDsl;
  ...
  private static boolean isLive(String brokerTarget) {
    return brokerTarget.endsWith("-live");
  }
