 ...
 *
 * <p>Mirrors {@link CrossTenantBrokerTargetValidator}: same {@link StrategyRegistry#list()} feed
 ...
 * {@code strategy_config} DB store in db-mode), so the boot gate validates exactly the config the
 * live read path will serve. A row that cannot load throws and the throw propagates (boot fails
 * closed) — never caught/skipped.
 *
 * <p>The ENUMERATION now comes from the same registry rather than a filesystem scan. Previously the
 * pairs were scanned out of the mounted {@code tenants/} tree, which on the live cluster held 4 of
 * 8 {@code (tenant, strategy)} pairs — so half the real-money strategies were never gated. In
...
public final class LiveRequiredGateValidator {
   ...
   * gate. No-op when the registry enumerates nothing. Config for each pair is read via {@code
   * registry.get}; that read throws on a missing/invalid row, and the throw propagates (boot fails
   * closed).
   *
   ...
   */
  public static void validate(StrategyRegistry registry, TenantRegistry tenantRegistry) {
