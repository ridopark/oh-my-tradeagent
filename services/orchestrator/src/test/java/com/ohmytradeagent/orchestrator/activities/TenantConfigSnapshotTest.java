...
/**
 * Regression guard for PLAN-2026-07-09: the three watchlist-only fields ({@code entry_mode}, {@code
 * watchlist_expiry_rule}, {@code gap_tolerance_pct}) are opt-in and null-when-absent, so a
 * copytrade StrategyConfig that never sets them must NOT carry them in its canonical config. Before
 * the fix they defaulted in the schema and leaked into copytrade's canonical config (and the {@code
 * /config} editor). The universal {@code enabled} field keeps its default and stays present.
 */
