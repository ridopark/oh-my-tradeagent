...
/**
 * Phase B1 controller slice test. {@link CopytradeFanoutController} is a plain jOOQ read against
 * the orchestrator {@code strategy_config} table. We back it with a jOOQ {@link MockDataProvider}
 * that (a) captures the rendered SQL + bind values so the enabled-predicate and the parameterized
 * strategy id are asserted, and (b) applies the same {@code IS DISTINCT FROM 'false'} + strategy
 * filter to an in-memory dataset so inclusion/exclusion (enabled true, enabled absent, exclude
 * enabled:false, exclude non-copytrade) is exercised end-to-end through the controller.
 */
class CopytradeFanoutControllerTest {
  ...
  @Test
  void returnsEnabledAndAbsentCopytradeTenants_excludesDisabledAndNonCopytrade() {
    List<Row> dataset =
        new ArrayList<>(
            List.of(
                ...
                new Row("delta", "other-strategy", "true"))); // non-copytrade → excluded
    CopytradeFanoutController controller =
        new CopytradeFanoutController(dslOver(dataset), STRATEGY);
  ...
  void queryUsesEnabledPredicate_andParameterizesStrategyId() {
    CopytradeFanoutController controller =
        new CopytradeFanoutController(dslOver(List.of()), STRATEGY);
  ...
  @Test
  void honorsConfiguredStrategyId_notCopytradeHardcoded() {
    String customStrategy = "some-other-copytrade";
    List<Row> dataset =
        new ArrayList<>(
            List.of(
    ...
    CopytradeFanoutController controller =
        new CopytradeFanoutController(dslOver(dataset), customStrategy);
