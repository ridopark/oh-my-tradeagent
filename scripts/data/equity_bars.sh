...
[[ "$START" =~ ^[0-2][0-9]:[0-5][0-9]$ ]]    || { echo "equity_bars: bad start (hh:mm utc): $START" >&2; exit 2; }
[[ "$END"   =~ ^[0-2][0-9]:[0-5][0-9]$ ]]    || { echo "equity_bars: bad end (hh:mm utc): $END" >&2; exit 2; }
[[ "$NS"    =~ ^[a-z0-9-]+$ ]]               || { echo "equity_bars: bad namespace: $NS" >&2; exit 2; }
