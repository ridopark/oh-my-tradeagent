// @generated by scripts/gen-config-field-manifest.py — DO NOT EDIT. Source: contract/schemas/strategy-config.json
/* eslint-disable */
...
export const STRATEGY_CONFIG_FIELDS: GeneratedConfigField[] = [
  ...
  },
  {
    ...
    options: [
      ...
      { value: "alpaca-live", label: "Alpaca-live" },
      { value: "tradier-paper", label: "Tradier-paper" },
      { value: "tradier-live", label: "Tradier-live" },
      { value: "ibkr-paper", label: "Ibkr-paper" },
      { value: "ibkr-live", label: "Ibkr-live" },
  ...
  },
  {
    ...
    kind: "number",
    description: "Issue #6 portfolio-level gate: max accepted BTO entries per UTC trading day. Reject with DAILY_TRADE_COUNT_EXCEEDED when today's count (from audit log: SignalAccepted with action=BTO) >= daily_trade_count. Opt-in: null disables the gate. Anti-overtrading circuit-breaker for volatile sessions.",
  },
  {
  ...
  },
  {
    field: "enabled",
    kind: "boolean",
    description: "Per-tenant strategy on/off toggle (operator requirement). When false the strategy is loaded but admits no new entries. Absent/null treated as true, so existing tenants are unaffected.",
  },
  {
    field: "entry_mode",
    kind: "string",
  ...
  },
  {
    ...
    kind: "number",
    description: "Reject STC signals older than this. Risk gate SIGNAL_TOO_OLD on exceed. Issue #3 (Phase 2a hardening): STC tolerates a larger window than BTO because exiting late is generally safer than entering late, but a 1800s default still permits adverse selection on partial fills. Defaults: 60s for STC. Any value above 120s is an explicit per-strategy override.",
  },
  {
  ...
  },
  {
    ...
    kind: "number",
    description: "Issue #6 portfolio-level gate: max concurrent positions on the same underlying ticker. Reject with SAME_UNDERLYING_LIMIT when count(open positions whose underlying == payload.ticker) >= same_underlying_count. Opt-in: null disables the gate. Prevents stacking convicted-author BTOs onto a single stock during high-vol windows.",
  },
  {
  ...
  },
  {
    ...
    kind: "number",
    description: "Issue #6 portfolio-level gate: max concurrent positions whose underlying maps to the same sector as payload.ticker. Reject with SECTOR_CONCENTRATION_EXCEEDED when count(open positions in payload.ticker's sector) >= sector_concentration_cap. Sector mapping comes from sector_overrides; unmapped tickers are treated as sector 'unknown' and exempt from the cap. Opt-in: null disables the gate.",
  },
  {
    field: "sector_overrides",
    description: "Issue #6: ticker → sector string mapping consulted by sector_concentration_cap. Unmapped tickers resolve to 'unknown' and are exempt from the sector gate. Per-strategy overrides keep the deployable config small while letting risk-tight strategies opt in to a curated map.",
  },
  {
    field: "skip_avg",
    kind: "boolean",
    description: "If true, AVG (average-down) actions are skipped by the workflow. Default true per PLAN.md Open Question #10.",
  },
  {
    field: "sl_pct",
    kind: "number",
    description: "Watchlist-trigger exit: hard stop as a fraction of entry premium. R = sl_pct * entry_premium; the -1R stop triggers when the live bid falls to entry_premium * (1 - sl_pct) and routes a MARKETABLE flatten (reason=stop_loss). Carried into PositionWorkflowInput.sl_pct. Opt-in: null/absent disables the premium TP/SL/trail exit (see tp_ratio). getVersion-gated.",
  },
  {
    field: "stc_intent_enforce",
    kind: "boolean",
    description: "Opt-in gate for STC close-intent enforcement (PLAN-2026-07-25-stc-intent-classifier). When true, a later phase consults copytrade-signal-payload.close_intent to arbitrate full-vs-partial STC sizing, with the keyword matcher as fallback. Null/absent = disabled (behavior-neutral). Spec-only field in this PR; runtime wiring lands separately, gated per-tenant.",
  },
  {
    field: "strategy_id",
    kind: "string",
  ...
  },
  {
    ...
    kind: "number",
    description: "Watchlist-trigger exit: fraction of the remaining position closed when the +tp_ratio*R take-profit first triggers. The unclosed remainder moves its stop to breakeven (entry_premium) and arms the chandelier trail (trail_giveback_pct) on the runner. Carried into PositionWorkflowInput.tp_partial_fraction. Default 0.5 in PositionWorkflowImpl when null and tp_ratio is set.",
  },
  {
    field: "tp_ratio",
    kind: "number",
